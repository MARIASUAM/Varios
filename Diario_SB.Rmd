...
En cualquier caso, *la agregación no se debería hacer por año natural, sino por ciclo biológico!* ¿cuándo es la medicion de COPLAS? 

"Since 1973, forestry technicians have evaluated the degree of infestation by PPM in all pine stands *at the end of the winter season (February–April)* [...]" --> COnsideremos 20 de marzo. 

If else para el primer año y el ultimo

INSTAR_BZ2$ciclo<-as.numeric(...)

```{r}
library(lubridate)

INSTAR_bz2$yday <- yday(INSTAR_bz2$fecha) # anadimos el dia del anio con el paquete lubridate
INSTAR_bz2$year <- year(INSTAR_bz2$fecha)


for(i in vector){commands}

INSTAR_bz2$biociclo <- rep("sinvalor")
ifelse((INSTAR_bz2$yday[2]>=50 && INSTAR_bz2$year[2]==1993), 
       INSTAR_bz2$biociclo[2] == 1, INSTAR_bz2$biociclo[2]==2)
...
for (i in unique(INSTAR_bz2$year)){
  if(INSTAR_bz2$year == min(INSTAR_bz2$year))
     {INSTAR_bz2$by <- ifelse(INSTAR_bz2$ciclo == 0, 
                  by, by + 1) 
       by <- by + 1}
  next
  }
...
for (i in 1:nrow(df)){ 
  ...
  df$by <- ifelse(df$year == start_year && df$ciclo == 0,
                      by, 
                      by + 1)
...
for (i in unique(df$year)){ 
  if(df$year == min(df$year))
     {df$by <- ifelse(df$ciclo == 0, 
                  by, by + 1) 
       by <- by + 1}
  next
  }
