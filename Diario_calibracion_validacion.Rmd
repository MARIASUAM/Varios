...
**Pregunta práctica**: Ya que COPLAS es en realidad una media con su varianza, entiendo que debe haber alguna manera de incluir esto en la regresión (de decirle a R cuáles son los valores de COPLAS reales, para que haga la regresión considerando media y varianza). ¿Alguna idea?

## Transformación log de datos INSTAR

**DUDA* qué base usamos para el log?
...
```{r}
COPLAS_SN<-read.csv("coplas_cortijuela.csv", header=TRUE, dec =".", sep=";")
COPLAS_SN_subset<-COPLAS[-c((1:6), 21), -c(2,3)] # Necesario? ver apuntes, comando para definir valores sin su pareja
```

## Correlaciones y regresiones

**DUDAS** lm o glm? cor y/o lm/glm?
